...
-- CQS per the ECAI mapping (Implementing Regulation (EU) 2016/1799) for long-term scales. The grade carries
-- its modifier (`BBB-`, `Baa2`); the CQS is set on the unmodified grade (modifiers do not cross a CQS boundary).
WITH r AS (
    SELECT *,
...
SELECT
    ...
    r.rating_grade || coalesce(r.rating_modifier, '') AS rating_grade,
    CASE WHEN rating_segment <> 'shortTerm' THEN
        CASE
            WHEN r.rating_grade IN ('AAA', 'AA', 'Aaa', 'Aa')  THEN 1
            WHEN r.rating_grade IN ('A')                       THEN 2
            WHEN r.rating_grade IN ('BBB', 'Baa')              THEN 3
            WHEN r.rating_grade IN ('BB', 'Ba')                THEN 4
            WHEN r.rating_grade IN ('B')                       THEN 5
            WHEN r.rating_grade IN ('CCC', 'Caa', 'CC', 'Ca', 'C', 'D', 'SD') THEN 6
        END
