...
class RunOutput:
    ...
    def sector_parameters(self) -> dict[tuple[str, str, str, int], dict[str, str]]:
        """Sectoral (GVA) satellite paths (scenario key sector_satellites): (segment, sector, scenario, year)."""
        return {(r["segment"], r["sector"], r["scenario"], int(r["year"])): r for r in self.rows("sector_parameters.csv")}
