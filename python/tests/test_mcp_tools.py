...
# -- sectoral (GVA) satellites in explain and diff ---------------------------------------------------------------
def test_explain_and_diff_sector_satellites(tmp_path):
    """NFC segments: explain lists the sectors with their own path (sector_parameters.csv) and the overview the
    sectoral-model shares; diff-runs names a moved sector path as a driver."""
    seg = "LOANS|NFC_SME_OTHER|BE"
    r = explain_result(GOLDEN, seg, "adverse", 3)
    by = {s["sector"]: s for s in r["sectors"]}
    assert by["F"]["pd_tr"] == "sectoral" and by["F"]["lgd_lr"] == "sectoral" and by["F"]["gva_key"] == "BE"
    assert by["D"]["pd_tr"] == "portfolio" and by["D"]["lgd_lr"] == "sectoral"
    assert [(p["scenario"], p["year"]) for p in by["F"]["path"]] == [("adverse", 3)]
    assert "Sectoral (GVA) satellites" in r["narrative"]
    us = explain_result(GOLDEN, "LOANS|NFC_SME_OTHER|US")
    assert all(s["gva_relative"] and s["gva_key"] == "EU" for s in us["sectors"]) and "EU GVA" in us["narrative"]
    assert explain_result(GOLDEN, "LOANS|GG|BE")["sectors"] == []
    o = explain_result(GOLDEN)
    assert o["sector_satellites"]["pd_tr_share"] > 0.9 and "sectoral PD/TR model" in o["narrative"]
    b = _copy_golden(tmp_path / "b")
    with open(b / "sector_parameters.csv") as f:
        rows = list(csv.DictReader(f))
    for row in rows:
        if row["segment"] == seg and row["sector"] == "F" and row["scenario"] == "adverse":
            row["pd12m_s1"] = f"{float(row['pd12m_s1']) * 2:.9f}"
    with open(b / "sector_parameters.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0]))
        w.writeheader()
        w.writerows(rows)
    _scale_segment(b, seg, 1.01)
    d = diff_runs(GOLDEN, b, segment=seg, scenario="adverse", year=2)
