...
8 + 9); off-balance items of the segment. Plus a short narrative. Without a segment: totals and top segments.

**`diff_runs`.** Per file, rows matched by key (segment / scenario / year, template row keys for the CR_* files),
numbers within `max(abs_tol, rel_tol * |value|)`; rows only in one run, changed rows, per-column changes and the
...
fields of the segment that changed (at t and t+1, with the `source`) and changed starting stocks. With a PD overlay
