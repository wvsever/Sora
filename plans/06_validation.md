...
A fixed test scenario (`tests/scenarios/test_eba2025.yaml` plus `scenarios/eba2025_macro.csv`) is committed alongside the golden files. The golden results depend only on synthetic inputs: the reference dataset, synthetic satellite coefficients and a synthetic ECB-style benchmark file (`tests/params/synthetic_ecb_benchmarks.csv`, generated by `tools/synthetic_ecb_benchmarks.py`). The benchmark application rules (10% coverage, portfolio level, no adjustment) can therefore be tested without the confidential ECB values.
