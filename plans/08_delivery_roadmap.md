...
| 5b Agent and calculator integration | Calculator REST client and IRB REA projection done (`--calculator`, `rea.csv`). `sora-mcp` done (stdio MCP server, optional `mcp` extra: describe, profile_source, test_mapping, validate_sim, reconcile, run_scenario, explain_result, diff_runs; read roots, metadata-only default with capped `include_values`, no shell, audit log, human approval of production mappings), with `sora-tools explain` and `sora-tools diff-runs` (exposure vs coverage attribution) and reconciliation controls (`mappings/cppbank/reconciliation.yaml`). Open: `/v1/parameters/credit` integration. |
